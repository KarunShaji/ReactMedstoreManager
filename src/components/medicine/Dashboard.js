    ...
    return (
        <div>
            ...
            <div className="container">
                ...
                <div>
                    <SearchMedicine setFilteredMedicine={handleFilteredMedicine} /><br />
                    <div className="container d-flex justify-content-center align-items-center">
                    
                            <Link to="/create" className="btn btn-dark mb-4">Add Medicine</Link>
                        
                    </div>
                    <div className="col">
                    {filteredMedicine.map(post => (
                        <MedicineListItem key={post.id} post={post} refresh={fetchMedicine} />
                    ))}
                    </div>
                </div>
                <div className="row">
                    <div> 
                        {!filteredMedicine.length && medicine.map(post => (
                            <MedicineListItem key={post.id} post={post} refresh={fetchMedicine} />
                        ))}
                    </div>
                </div>
